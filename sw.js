...
const CACHE_NAME = 'giolynk-v4';

// All paths are RELATIVE (no leading /) so they resolve from the SW's own scope.
// This works whether the site is deployed at root (/) or a subdirectory (/geevox/ etc.)
const STATIC_ASSETS = [
  './',
  './index.html',
  './main.css',
...
// Helper: get the base URL matching the SW scope (works in subdirectory deploys)
function scopeBase() {
...
self.addEventListener('push', event => {
  ...
  // Use scope-relative paths for icons so they work in subdirectory deploys
  const base = scopeBase();
  const icon = notification.icon || (base + '/icon-192.png');
  const badge = notification.badge || (base + '/icon-72.png');

  // Build notification click action URL (relative to scope)
  let clickUrl = base + '/';
  if (fcmData.link) {
    clickUrl = fcmData.link;
  } else if (fcmData.page) {
    if (fcmData.postId) {
      clickUrl = base + '/?page=post-detail&postId=' + fcmData.postId;
    } else if (fcmData.chatId) {
      clickUrl = base + '/?page=chat-view&chatId=' + fcmData.chatId;
    } else if (fcmData.groupId) {
      clickUrl = base + '/?page=group&groupId=' + fcmData.groupId;
    } else if (fcmData.userId) {
      clickUrl = base + '/?page=user-profile&userId=' + fcmData.userId;
    } else {
      clickUrl = base + '/?page=' + fcmData.page;
    }
  }
...
self.addEventListener('notificationclick', event => {
  ...
  const clickUrl = event.notification.data?.url || (scopeBase() + '/');
